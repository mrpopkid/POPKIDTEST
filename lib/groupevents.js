...
const getProfilePicture = async (malvin, jid) => {
    try {
        return await popkid.profilePictureUrl(jid, 'image');
    } catch {
        return fallbackPP;
    }
};
...
const GroupEvents = async (popkid, update) => {
    try {
        if (!isJidGroup(update.id)) return;

        const metadata = await popkid.groupMetadata(update.id);
        ...
        for (const participant of update.participants) {
            ...
            const userPp = await getProfilePicture(popkid, userId);
            const groupPp = await getProfilePicture(popkid, update.id);
...
`.trim();
                ...
                await popkid.sendMessage(update.id, {
                    image: { url: userPp },
                    caption: welcomeMsg,
                    mentions: [userId],
